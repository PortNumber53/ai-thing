...
export async function getContent(url_path: string): Promise<any | null> {
  try {
    console.log('Fetching content for url_path:', url_path);
...
export async function saveContent(url_path: string, payload: { title?: string; body?: string }) {
  // Validate input
  if (!url_path) {
  ...
  try {
    ...
    const existingRecord = await xata.db.contents.filter({ url_path }).getFirst();
    ...
    if (existingRecord) {
      ...
      savedRecord = await xata.db.contents.update(existingRecord.xata_id, contentToSave);
    } else {
      // Create new record
