...
export async function loader({ params }: LoaderFunctionArgs) {
  ...
  if (!project_path) {
    throw new Response("Project path is required", { status: 400 });
  }

  const fullPath = `portfolio/${project_path}`;
  ...
  // If project doesn't exist, return an empty template
  if (!project) {
    return json({
      project: {
        title: project_path.replace(/-/g, " "),
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (!project) {
    ...
    await xata.db.contents.update(project.xata_id, {
      title,
      content: { current: content },
