...
file_patch_tool = AITool(
  ...
  high
The function currently returns {"success": True, ...} even if some or all of the patches failed to apply. Refactor the function to track the success status across all patches and return success: False if any patch fails. Also, handle per-patch errors gracefully without aborting the entire batch, aborting only on fatal errors like the patch command not being found.
