...
class FileFullWriteTool:
    ...
    def get_definition(self) -> Dict[str, Any]:
        ...
        return {
            'function_declarations': [{
                ...
                'description': 'Writes content to a specified file. Can override existing files or preserve them by renaming with a timestamp.',
                'parameters': {
                    'type': 'OBJECT',
                    'properties': {
                        'file_path': {
                            'type': 'STRING',
                            'description': 'Path to the file to write. Can be absolute or relative to the base path.'
                        },
                        'file_contents': {
                            'type': 'STRING',
    ...
    def get_summary(self) -> str:
        ...
        return "Writes content to a file, with options to override or preserve existing files."
...
"""
    ...
    def execute(self, file_path: str, file_contents: str, override: Optional[bool] = False, preserve_original: Optional[bool] = False) -> Dict[str, Any]:
        ...
        if self.base_path and not os.path.isabs(file_path):
            resolved_file_path = os.path.join(self.base_path, file_path)
        else:
        ...
        try:
            ...
            if file_exists:
                if preserve_original:
                    ...
                    backup_file_path = f"{resolved_file_path}.{timestamp}.bak"
                    try:
                        os.rename(resolved_file_path, backup_file_path)
                        original_path_preserved = backup_file_path
                    except Exception as e:
                        return {
                            "file_path": resolved_file_path,
                            "status": "error",
                            "message": f"Failed to preserve original file: {str(e)}"
                        }
                elif not override:
                    return {
                        "file_path": resolved_file_path,
                        "status": "error",
                        "message": "File exists and override is false. Set override to true or preserve_original to true."
                    }
            
            # At this point, either the file didn't exist, was preserved, or override is true.
...
if __name__ == '__main__':
    writer_tool = FileFullWriteTool(base_path='./test_files')
    ...
    if res1['status'] == 'success' and os.path.exists(os.path.join('./test_files', 'new_file.txt')):
        with open(os.path.join('./test_files', 'new_file.txt'), 'r') as f:
    ...
    if res5['status'] == 'success' and os.path.exists(os.path.join('./test_files', 'another_new_file.txt')):
        with open(os.path.join('./test_files', 'another_new_file.txt'), 'r') as f:
            print(f"Content: {f.read()}")
    print("-"*20)
    ...
    if res6['status'] == 'success' and os.path.exists(os.path.join('./test_files', 'subdir/deep_file.txt')):
        with open(os.path.join('./test_files', 'subdir/deep_file.txt'), 'r') as f:
            print(f"Content: {f.read()}")
    print("-"*20)
    ...
    # print("Cleaned up test_files directory.")
